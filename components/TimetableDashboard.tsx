...
export default function TimetableDashboard({
  ...
  initialPortalMode?: 'client' | 'admin';
  initialViewMode?: 'landing' | 'app';
} = {}) {
  const [isMounted, setIsMounted] = useState(false);

  // --- Theme Toggle State ---
  const [theme, setTheme] = useState<'dark' | 'light'>('dark');
  const [currentViewMode, setCurrentViewMode] = useState<'landing' | 'app'>(initialViewMode);
  const [saasPortalMode, setSaasPortalMode] = useState<'client' | 'admin'>(initialPortalMode);
  ...
  // --- Core Application States (Chargées dynamiquement depuis Supabase) ---
  ...
  const [saasSettings, setSaasSettings] = useState<SaaSGlobalSettings>({
  ...
  const DEFAULT_FALLBACK_PLAN: SaaSPlan = useMemo(() => ({
    id: 'plan_trial',
  ...
  const DEFAULT_FALLBACK_CLIENT: SaaSClient = useMemo(() => ({
    id: currentClientId || 'default_client',
    schoolName: schoolName || 'Mon Établissement',
    logoIcon: schoolLogoIcon || 'GraduationCap',
    adminName: currentUserEmail ? currentUserEmail.split('@')[0] : 'Administrateur',
    adminEmail: currentUserEmail,
    phone: '',
    cityCountry: 'Sénégal',
    planId: userPlanId || 'plan_trial',
    status: 'active',
    ...
    classesCount: 0,
    teachersCount: 0
  }), [currentClientId, schoolName, schoolLogoIcon, currentUserEmail, userPlanId]);
  ...
  const currentClient = useMemo(() => {
    ...
    if (found) {
      return {
        ...found,
        planId: userPlanId || found.planId || 'plan_trial',
        classesCount: classes.length,
        teachersCount: teachers.length
      };
    }
    return {
      ...(saasClients[0] || DEFAULT_FALLBACK_CLIENT),
      planId: userPlanId || 'plan_trial',
      classesCount: classes.length,
      teachersCount: teachers.length
    };
  }, [saasClients, currentClientId, classes.length, teachers.length, DEFAULT_FALLBACK_CLIENT, userPlanId]);
  ...
  const maxGenerations = useMemo(() => {
    return currentPlan.maxGenerations || 9999;
  }, [currentPlan]);
...
3. Renseignez votre clé : ${keyToSend}`;
  ...
  // Reset / Refresh from Database helper
      ...
      try {
        ...
        if (currentUserId) {
          await dbSaveTimetable(
            currentUserId,
            `Emploi du temps - ${new Date().toLocaleDateString('fr-FR')}`,
            data.timetable,
            data.unscheduled,
            data.score,
        ...
        if (data.isFullyScheduled) {
          ...
          triggerNotification(`Généré et sauvegardé (${data.score}% planifié). Vérifiez les alertes.`, "info");
        }
      } catch (err: any) {
        console.error(err);
  ...
  const handleAddSubject = async (e: React.FormEvent) => {
    ...
    if (currentUserId) {
      const created = await dbAddSubject(currentUserId, newSubName.trim());
      if (created) {
        setSubjects(prev => [...prev, created]);
        setNewSubName('');
        triggerNotification(`Matière "${created.name}" enregistrée dans la base de données !`);
      } else {
        triggerNotification("Erreur lors de l'enregistrement de la matière en base.", "error");
      }
    } else {
  ...
  const handleSaveTeacher = async (e: React.FormEvent) => {
    ...
    if (editingTeacherId) {
      ...
      if (currentUserId) {
        await dbUpdateTeacher(editingTeacherId, payload);
      }
      ...
      setTeachers(updated);
      triggerNotification("Fiche enseignant mise à jour dans la base de données !");
    } else {
      // Check plan limits
      ...
      if (currentUserId) {
        const created = await dbAddTeacher(currentUserId, newTeachData);
        if (created) {
          setTeachers(prev => [...prev, created]);
          triggerNotification(`Enseignant "${created.name}" enregistré dans la base de données !`);
        }
      } else {
  ...
  const handleSaveClass = async (e: React.FormEvent) => {
    ...
    if (editingClassId) {
      ...
      if (currentUserId) {
        await dbUpdateClass(editingClassId, payload);
      }
      ...
      setClasses(updated);
      triggerNotification("Fiche classe mise à jour dans la base !");
    } else {
      // Check plan limits
      ...
      if (currentUserId) {
        const created = await dbAddClass(currentUserId, newClassPayload);
        if (created) {
          setClasses(prev => [...prev, created]);
          triggerNotification(`Classe "${created.name}" créée dans la base de données !`);
        }
      } else {
  ...
  // --- HANDLERS DÉDIÉS AU MODULE PLANIFICATION PÉDAGOGIQUE ---
  const handleAddTeacherFromPlanning = async (teacherData: Omit<Teacher, 'id'>): Promise<Teacher | null> => {
    if (teachers.length >= currentPlan.maxTeachers) {
      triggerNotification(`Limite d'enseignants atteinte pour votre formule (${currentPlan.maxTeachers} max).`, "error");
      openUpgradeModal("Quota d'Enseignants Atteint", `Votre formule actuelle est limitée à ${currentPlan.maxTeachers} professeurs.`);
      return null;
    }
    if (currentUserId) {
      const created = await dbAddTeacher(currentUserId, teacherData);
      if (created) {
        setTeachers(prev => [...prev, created]);
        triggerNotification(`Professeur "${created.name}" ajouté avec succès !`);
        return created;
      }
    }
    const localTeacher: Teacher = {
      id: `teach-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      ...teacherData
    };
    setTeachers(prev => [...prev, localTeacher]);
    triggerNotification(`Professeur "${localTeacher.name}" ajouté !`);
    return localTeacher;
  };

  const handleAddClassFromPlanning = async (classData: Omit<ClassGroup, 'id'>): Promise<ClassGroup | null> => {
    if (classes.length >= currentPlan.maxClasses) {
      triggerNotification(`Limite de classes atteinte pour votre formule (${currentPlan.maxClasses} max).`, "error");
      openUpgradeModal("Quota de Classes Atteint", `Votre formule actuelle est limitée à ${currentPlan.maxClasses} classes.`);
      return null;
    }
    if (currentUserId) {
      ...
      if (created) {
        setClasses(prev => [...prev, created]);
        triggerNotification(`Classe "${created.name}" créée avec succès !`);
        return created;
      }
    }
    const localClass: ClassGroup = {
      id: `cls-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      ...classData
    };
    setClasses(prev => [...prev, localClass]);
    triggerNotification(`Classe "${localClass.name}" créée !`);
    return localClass;
  };

  const handleAddSubjectFromPlanning = async (name: string): Promise<Subject | null> => {
    const trimmed = name.trim();
    if (!trimmed) return null;
    const existing = subjects.find(s => s.name.toLowerCase() === trimmed.toLowerCase());
    if (existing) return existing;
    if (currentUserId) {
      const created = await dbAddSubject(currentUserId, trimmed);
      if (created) {
        setSubjects(prev => [...prev, created]);
        triggerNotification(`Matière "${created.name}" enregistrée !`);
        return created;
      }
    }
    ...
    triggerNotification(`Matière "${localSub.name}" enregistrée !`);
    return localSub;
  };
